...
public function mailer($decryptedBody, $id)
{
    try {
        $name = Auth::user()->name;
        $user = User::find($id);
        $phonenumber = $user->phonenumber;
        $concatenated = "+63${phonenumber}";

        // Call the verification function to verify the phone number
        // $verificationResult = $this->verifyPhoneNumber($concatenated);

        // if (!$verificationResult) {
        //     // If verification fails, log the error and return false
        //     \Log::error('Phone number verification failed.');
        //     return false;
        // }
        ...
        // Send the message if verification is successful
...
//     try {
//         // Create a validation request to verify the phone number
//         $twilio->validationRequests
//             ->create($phoneNumber, ["friendlyName" => "My Phone Number"]);

//         // Phone number verification request successfully created
//         return true;
//     } catch (\Exception $e) {
//         // Handle error
    ...
    public function send(Request $request)
{
    ...
    if (!$error->status) {
    $encryptedMessage = Crypt::encryptString(htmlentities(trim($request['message']), ENT_QUOTES, 'UTF-8'));
    ...
    // Decrypt the 'body' attribute before parsing the message
    $decryptedMessage = Crypt::decryptString($message->body);
    $message->body = $decryptedMessage;

    // Now parse the decrypted message data
    ...
    $this->mailer($decryptedMessage,$id);
...
public function fetch(Request $request)
{
    ...
    $response = [
        'total' => $totalMessages,
        'last_page' => $lastPage,
        'last_message_id' => $messages->isEmpty() ? null : $messages->last()->id,
        'messages' => '',
    ];
    ...
    foreach ($messages->reverse() as $message) {
        // Decrypt the 'body' attribute of the message
        $message->body = Crypt::decryptString($message->body);
        
        // Generate the message card using the decrypted message
